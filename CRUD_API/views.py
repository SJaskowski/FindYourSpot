...
from .models import Apartment, Room, Images, Custom_User, Address, Opinion, Advert
...
class PostedAdvert(APIView):
    def put(self, request):
        ...
        try:
            if type == 'apartment':
                apartment = Apartment.objects.get(id=id)
                Advert.objects.create(apartment=apartment)
            elif type == 'room':
                room = Room.objects.get(id=id)
                Advert.objects.create(room=room)
    ...
    def get(self, request):
        ...
        try:
            advert = list(Advert.objects.filter(id=id).values())
        except ObjectDoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)

        return Response({'opinons': advert})
    ...
    def delete(self, request):
        try:
            ...
            Response(status=Advert.HTTP_400_BAD_REQUEST)
        try:
            tmp_advert = Advert.objects.get(id=id)
            tmp_advert.delete()
            return Response(status=status.HTTP_200_OK)
        except (ObjectDoesNotExist, UnboundLocalError):
            return Response(status=status.HTTP_404_NOT_FOUND)
    ...
    def patch(self, request):
        ...
        try:
            tmp_advert = Advert.objects.get(id=id)
            for new_data in patch_data.items():
                setattr(tmp_advert, new_data[0],new_data[1])
                tmp_advert.save()
            return Response(status=status.HTTP_200_OK)
        except ObjectDoesNotExist:
            return Response(status=status.HTTP_404_NOT_FOUND)